...
namespace Kursovaya_work.Controllers
{
    public class AccountController : BaseController
    {
        ...
        public AccountController(Context context, IOptions<AuthOptions> options, IHostEnvironment hostEnvironment)
        {
        ...
        private async Task<bool> RegisterUser(string username, string name, string lastname, string passport, string password, UserRole role, IFormFile file)
        {
        ...
        {
            if (context.Flight.Any(p => p.Name == name))
                return false;
        ...
        public async Task<IActionResult> Register(RegisterModel model)
        {
            if (!ModelState.IsValid)
                return View(model);
            if (await RegisterUser(model.Username,model.Name,model.LastName,model.Passport, model.Password, UserRole.User, model.Avatar))
                return Redirect("/Home/Index");
            else
            {
                ModelState.AddModelError("Username", "Login in use");
                return (View(model));
            }
        }
        ...
        public async Task<IActionResult> Login(LoginModel model)
        {
            ...
            {
                ModelState.AddModelError("Username", "Invalid login or password");
                return View(model);
            }
